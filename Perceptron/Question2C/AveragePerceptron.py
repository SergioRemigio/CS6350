...
This file contains the implementation of the Average perceptron algorithm.
'''
import csv
import math
import random
...
def get_average_separating_hyperplane(D, w, T, r):
    """
    ...
    for t in range(T):
        ...
        for xi in D:
            ...
            # a ← a + w
            a = add_two_vectors(a, w)
        # Update the learning rate
        r = r / 2
    return a


def print_prediction_success_percentage(a, r, test_examples):
    """
    After the perceptron algorithm has found an average separating hyperplane a given the training examples,
    the file prints to the console:
    the separating hyperplane as a
    the average prediction success of the given test examples
    :param a: separating hyperplane u
    :param test_examples: Test examples to predict
    """
    success_count = 0
    for xi in test_examples:
        prediction = get_prediction(a, xi)
    ...
    print("Average separating hyperplane a: " + str(a))
...
if __name__ == "__main__":
    ...
    # Learning Rate
    r = .5
    a = get_average_separating_hyperplane(D, w, T, r)
    normalize(a)

    test_examples = import_examples("test.csv")
    print_prediction_success_percentage(a, r, test_examples)
